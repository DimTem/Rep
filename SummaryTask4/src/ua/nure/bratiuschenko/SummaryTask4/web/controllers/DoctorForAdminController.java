...
public class DoctorForAdminController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		if (request.getParameter("surnsubmit") != null ) {
			List<Doctor> doctorsS = null;
			try {
				doctorsS = doctorDAO.findTen();
			} catch (DBException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			String doctorsTableS = TableGenerator.generateDoctorsTable(doctorsS);
			response.getWriter().print(doctorsTableS);
		}
		
		if (request.getParameter("surnsubmit") != null ) {
			List<Doctor> doctorsS = null;
			try {
				doctorsS = doctorDAO.findNine();
			} catch (DBException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		List<Doctor> doctorsS = null;
		try {
			doctorsS = doctorDAO.findOne();
		} catch (DBException e) {
			LOG.error("can not generate doctors table", e);
		}
		String doctorsTableS = TableGenerator.generateDoctorsTable(doctorsS);
		if (request.getParameter("surnsubmit") != null ) {
			String doctorSelect = request.getParameter("selectdoctor");
			response.getWriter().print(doctorsTableS);
		}
		...
		response.getWriter()
				.print("</br>&nbsp;&nbsp;&nbsp;<a href=/SummaryTask4/Doctor>BACK</a>&nbsp;&nbsp;&nbsp;");
		
		response.getWriter()
		.print("<a href=/SummaryTask4/jsp/admin/Doctors/showDoctorsBySurname.jsp>Show doctors by surname</a>&nbsp;&nbsp;&nbsp;<br>");
